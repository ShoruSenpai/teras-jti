...
export async function initSession(type) {
        ...
        (err) => {
          switch (err.code) {
            case err.PERMISSION_DENIED:
              reject(
                new Error(
                  'Akses ditolak! Mohon aktifkan izin lokasi pada pengaturan browser.',
                ),
              );
              break;
            case err.POSITION_UNAVAILABLE:
              reject(
                new Error(
                  'Informasi lokasi tidak tersedia. Pastikan GPS HP kamu sudah aktif.',
                ),
              );
              break;
            case err.TIMEOUT:
              reject(
                new Error(
                  'Waktu permintaan lokasi sudah habis. Silahkan coba lagi.',
                ),
              );
              break;
            default:
  ...
  try {
    const coords =
      type === 'dine-in' ? await getLocation() : { lat: null, long: null };
