...
const startProccess = async () => {
  ...
  const result = await initSession("dine-in");

  if (result.success) {
    router.push(`/dine-in/${result.token}/menu`);
  } else {
    if (result.message.toLowerCase().includes("Waktu") && retryCount.value < 1) {
      console.log("timeout. Menghapus Cookie...");
      removeCookie(`session_dine-in`);
      retryCount.value++;
      startProccess();
    } else {
...
onMounted(async () => {
  setTimeout(() => {
    if (isValidating.value) showInstruction.value = true;
  }, 2000);
